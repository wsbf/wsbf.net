...
function authenticate_logbook($mysqli)
{
	...
	// Note: ID "13" is computer engineer 2, since no two users are allowed to 
	// have the same senior staff position. This should probably be fixed.
	if ( get_position($mysqli) == "8" || get_position($mysqli) == "13"
		 || get_position($mysqli) == "1")  {
		return;
	}
